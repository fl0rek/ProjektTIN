...
if [[ -z "$TRAVIS_BUILD" ]] ; then # there is no c++14 on travis so build fails

mkdir "$BUILD_DIR/GameAPP"
pushd GameAPP
echo Building GameAPP
qmake --qt=qt5 "$PROJECT_ROOT/GameAPP" || QT_SELECT=qt5 qmake "$PROJECT_ROOT/GameAPP"
make -j $THREAD_COUNT
popd
...
qmake --qt=qt5 "$PROJECT_ROOT/ChatAPP" || QT_SELECT=qt5 qmake "$PROJECT_ROOT/ChatAPP"
make -j $THREAD_COUNT
popd
